...
#   when the data table is printed the column names are also printed at the botton
...
#--------
# %chin% - for character vectors ( much faster than %in% )
#--------
...
dt[ , .( DBP.mean=mean(diastBP), SBP.mean=mean(systBP)) , by = .(ID) ] 
...
dt[ age <=50 , .(mean_stay = mean(difftime( discharge, admission ))), by= ID ]
